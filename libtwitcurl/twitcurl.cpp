...
bool twitCurl::timelineHomeGet( const std::string sinceId )
{
    ...
    if( sinceId.length() )
    {
        buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_MAXID + sinceId;
    }
...
*
*--*/
bool twitCurl::timelineUserGet(const long int sinceId,
                                ...
                                const bool isUserId )
{
    ...
    if( includeRetweets )
    {
        ...
        buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_NOCRETWEETS;
    }
    ...
    if( sinceId>0 )
        {
          std::stringstream tmpStrm;
                    const std::string MAXID = "max_id=";
                  tmpStrm<<twitCurlDefaults::TWITCURL_URL_SEP_AMP + MAXID << sinceId;
...
void twitCurl::getLastWebResponse( std::string& outWebResp )
{
    ...
    {
        outWebResp.assign( m_callbackData);
